...
namespace SegmentTree {
  ...
  struct Segment {
    int L, R, mid;
    int l, r;
    int v; // left color; right color; segment count
    int tag;       // change to
  ...
  struct T { // Funcs, for convince
    static void _b(Segment &x) {
      x.mid = x.L + x.R >> 1;
      if (x.L == x.R) return x.vi(vAlign[x.L]);
      _b(a[x.l = ++c] = {Lg(x)});
      _b(a[x.r = ++c] = {Rg(x)});
      x.up();
    }
    static inline void B(int n) { _b(a[c = 1] = {1, n}); }
    static int _q(Segment &x, int L, int R) {
...
namespace Composer {
  ...
  void _bb(int o, int t) {
    top[o] = t; dfn[o] = ++tim; vAlign[dfn[o]] = v[o];
    if (son[o] == -1) return;
    _bb(son[o], t);
    for (int e=hed[o]; e; e=E[e].nxt) {
      int x = E[e].to;
